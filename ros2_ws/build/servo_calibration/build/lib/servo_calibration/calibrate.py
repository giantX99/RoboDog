...
class ServoConvert():
    ...
    '''
    def __init__(self, id=1, center=306, direction=1, max=4095, min=0):
        # max=520, min=83
        self.value      = center
        self._center    = center
        self._dir       = direction
...
class SpotMicroServoControl(Node):
    def __init__(self):
        ...
        self.servos = {
            ...
            6   :   ServoConvert(id=6, direction=-1, max=86, min=576), 
            7   :   ServoConvert(id=7, direction=-1, max=86, min=576), 
            8   :   ServoConvert(id=8, direction=1, max=566, min=206, center=376), # max = 566 ; min = 206 ; dir = 1 ; center = 376 
            9   :   ServoConvert(id=9, direction=-1, max=86, min=576),
            10   :   ServoConvert(id=10, direction=-1, max=86 , min=576), 
            11   :   ServoConvert(id=11, direction=1, max=106, min=436, center=266), # max = 106 ; min = 436 ; dir = -1 ; center = 266
        }
    ...
    def send_servo_msg(self):
        for servo_key, servo_obj in self.servos.items():
            ...
            self._servo_array_msg.servos[servo_obj.id].value = float(servo_obj.value)
            self.get_logger().info(f"Sending to {servo_obj.id + 1} command {servo_obj.value}")
