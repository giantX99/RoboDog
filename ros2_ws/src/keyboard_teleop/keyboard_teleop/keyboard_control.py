#!/usr/bin/python
"""
Class for sending keyboard commands to the Motion Command package Finite State Machine controller
it sends control velocity messages, yaw rate, and fsm events 
"""
import rclpy
...
class KeyboardControl(Node):
    def __init__(self):
        ...
        self._angle_cmd_msg.x = self.zero
        self._angle_cmd_msg.y = self.zero
        self._angle_cmd_msg.z = self.zero

        self._vel_cmd_msg = Twist()
        self._vel_cmd_msg.linear.x = self.zero
        self._vel_cmd_msg.linear.y = self.zero
        self._vel_cmd_msg.linear.z = self.zero
        self._vel_cmd_msg.angular.x = self.zero
        self._vel_cmd_msg.angular.y = self.zero
        self._vel_cmd_msg.angular.z = self.zero
        ...
        self.get_logger().info("Setting up the Keyboard Teleop Node...")
        ...
        self.get_logger().info("Keyboard Teleop node publishers corrrectly initialized")

        # Setup terminal input reading, taken from teleop_twist_keyboard
        self.settings = termios.tcgetattr(sys.stdin)

        self.get_logger().info("Keyboard Teleop node initialization complete")
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            if userInput not in valid_cmds:
                self.get_logger().warn(f"Invalid keyboard command entered: {userInput}")
            else:
                if userInput == 'quit':
                    self.get_logger().info("Exiting keyboard teleop node...")
                    break
                    ...
                    while (1):
                        ...
                        if userInput == 'u':
                            ...
                            self.get_logger().warn(f"Invalid keyboard command issued in angle command mode: {userInput}")
                        else:
                            if userInput == 'w':
...
def main(args=None):
    ...
    key_control = KeyboardControl()
    try:
        key_control.run()
        key_control.destroy_node()
        rclpy.shutdown()
